...
export function NodeSocket({
    ...
    return (
        <div
            className={`relative flex items-center ${isInput ? "" : "justify-end"} ${children ? "flex-1 min-w-0" : ""} ${className || ""}`}
        >
            <Tooltip>
                <TooltipTrigger asChild>
                    <Handle
                        ...
                        id={id}
                        className={
                            `absolute! ${isInput ? "left-0! -ml-3" : "right-0! -mr-3"} ` +
                            (elongated
            ...
            {isInput ? (
                <>
                    {(label || right) && (
                        <span
                            className={`text-[10px] font-medium uppercase text-muted-foreground flex items-center gap-1 ${right ? "flex-1 min-w-0" : "shrink-0"}`}
                            style={labelStyle}
                        >
                            {right ?? label}
                        </span>
                ...
                <>
                    ...
                    {(label || right) && (
                        <span
                            className={`text-[10px] font-medium text-muted-foreground flex items-center gap-1 ${right ? "flex-1 min-w-0" : "shrink-0"}`}
                            style={labelStyle}
                        >
                            {right ?? label}
                        </span>
