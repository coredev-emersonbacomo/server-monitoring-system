...
export function ClientCard({ client, onDelete }: ClientCardProps) {
    const isAssignedToCurrentUser = Boolean(client.is_assigned_to_current_user);
        ...
        >
            <div className="relative size-full rounded-lg border bg-card border-border p-6 shadow-xs flex flex-col items-center font-sans gap-3 transition-all hover:shadow-md hover:border-border/80">
                ...
                <div className="text-center w-full flex flex-col items-center gap-1 min-w-0 px-2">
                    <h3
                        className="font-semibold text-foreground text-base tracking-tight truncate w-full"
                        title={client.name}
                    >
                        {client.name}
                    </h3>
                    {client.description && (
                        <p className="text-xs text-muted-foreground text-center line-clamp-2 wrap-break-word w-full">
                            {client.description}
                        </p>
                    )}
                </div>
